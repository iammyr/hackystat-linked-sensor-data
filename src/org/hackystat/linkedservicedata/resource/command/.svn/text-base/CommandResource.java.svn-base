...
/**
 * Resource representing a command-line command.
 *
 * @author Myriam Leggieri.
 *
 */
public class CommandResource extends LiSeDCommandResource {

  public CommandResource(Context context, Request request, Response response) {
    ...
    if (media.equals(MediaType.APPLICATION_ALL) || media.equals(MediaType.TEXT_ALL)
      ...
      try {
        ...
        if (!validateUserIsAdmin() && !validateUserIsUriUser()) {
          if (!isSDAccessible(client.getUser(uriUser))) {
            return null;
          }
        }
        ...
        logRequest(resourceName, commandName);
